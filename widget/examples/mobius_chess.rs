...
fn emit_sequence(buf: &mut Buffer, x: u16, y: u16, sequence: &str) {
    if let Some(cell) = buf.cell_mut((x, y)) {
        ...
        // The escape prefix prints nothing; only the retained symbol occupies
        // the cell. Tell the diff so it does not skip the sequence's string
        // width in following cells (ratatui-core >= 0.1.2 semantics).
